pipeline {
    ...
    stages {
        ...
        stage('Wait for Initialization') {
            steps {
                script {
                    echo "Waiting for 60 seconds to allow the container to initialize..."
        ...
        stage('Test helm deplyment Apps') {
            steps {
                catchError(buildResult: 'FAILURE', stageResult: 'FAILURE') {
                    ...
                    bat 'python "Testing Scripts/backend_testing.py"'
                    bat 'python "Testing Scripts/frontend_testing.py"'
                    bat 'python "Testing Scripts/combined_testing-k8s.py"'
        ...
        stage('Clean helm installs') {
            steps {
                catchError(buildResult: 'FAILURE', stageResult: 'FAILURE') {
                    echo 'Cleaning up environment...'
                    bat "helm uninstall ${RELEASE_BACKEND}"
                    bat "helm uninstall ${RELEASE_FRONTEND}"
                    bat "helm uninstall ${RELEASE_DATABASE}"
                }
            }
        }
    }
